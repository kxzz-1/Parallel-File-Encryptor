/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Public surface (declared in ui_telemetry.h):
     ...
     void     telemetry_tick         (App *)   -- called by 60-FPS timer
   ═══════════════════════════════════════════════════════════════════════════ */
...
static inline void set_panel  (cairo_t *cr, double a) { cairo_set_source_rgba(cr, CLR_BG_PANEL, a); }
...
/* Process accent colours — one per MPI rank */
static const double PROC_COLORS[8][3] = {
    {0.000, 0.831, 1.000},   /* 0  cyan    */
    {0.486, 0.227, 0.929},   /* 1  purple  */
    {0.000, 0.898, 0.627},   /* 2  mint    */
    {0.961, 0.620, 0.043},   /* 3  amber   */
    {0.941, 0.271, 0.271},   /* 4  red     */
    {0.200, 0.600, 1.000},   /* 5  sky     */
    {0.600, 0.230, 0.930},   /* 6  violet  */
    {0.000, 0.700, 0.530},   /* 7  teal    */
};
...
/* ── glow helper: draws a soft radial bloom around (cx,cy) ─────────────── */
static void draw_glow(cairo_t *cr, double cx, double cy,
                      double radius, double r, double g, double b,
                      double peak_alpha) {
    cairo_pattern_t *pat = cairo_pattern_create_radial(cx, cy, 0,
                                                        cx, cy, radius);
...
/* ── scanline texture overlay (adds cyberpunk grain) ───────────────────── */
...
/* ── grid background ────────────────────────────────────────────────────── */
static void draw_grid(cairo_t *cr, double W, double H,
...
gboolean telemetry_draw_split(GtkWidget *w, cairo_t *cr, App *a) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        neon_label(cr, rank_s, nx + node_w/2 - 8, node_y + 20,
                   11, c[0], c[1], c[2], 1);
        ...
        double badge_r = 0.392, badge_g = 0.455, badge_b = 0.549;
        double badge_a = 0.4;
        if (state == PROC_RECEIVED)   { badge = "RECV"; badge_r=0.961; badge_g=0.620; badge_b=0.043; badge_a=0.9; }
        if (state == PROC_PROCESSING) {
            double blink = 0.7 + 0.3 * sin(tick * 0.18 + i);
            badge = "WORK"; badge_r=c[0]; badge_g=c[1]; badge_b=c[2]; badge_a=blink;
        }
        if (state == PROC_DONE)       { badge = "DONE"; badge_r=0.0; badge_g=0.898; badge_b=0.627; badge_a=0.9; }
        ...
        /* badge backdrop */
        cairo_set_source_rgba(cr, badge_r, badge_g, badge_b, 0.12);
        rounded_rect(cr, bx-4, by-11, bex.width+8, 14, 3);
        cairo_fill(cr);
        cairo_set_source_rgba(cr, badge_r, badge_g, badge_b, badge_a);
        ...
        /* data-flow connector dots between nodes */
        if (i < n-1) {
...
/* ═══════════════════════════════════════════════════════════════════════════
   2.  THREAD HEATMAP — OpenMP Processing Cells
   ═══════════════════════════════════════════════════════════════════════════ */
gboolean telemetry_draw_heatmap(GtkWidget *w, cairo_t *cr, App *a) {
    ...
    int cols = (n <= 8) ? n : (n <= 16) ? 8 : (n <= 24) ? 8 : 8;
    ...
    for (int i = 0; i < n; i++) {
        ...
        switch (state) {
            case THREAD_ACTIVE:
                cairo_set_source_rgba(cr, 0.00, 0.38, 0.55, 0.9); break;
            case THREAD_DONE:
                cairo_set_source_rgba(cr, 0.00, 0.27, 0.20, 0.9); break;
            default:
                cairo_set_source_rgba(cr, 0.1, 0.12, 0.2, 0.8);
        }
        cairo_fill(cr);
        ...
        /* ── active cell: inner highlight + top bar ── */
        ...
        switch (state) {
            ...
            case THREAD_DONE:
                cairo_set_source_rgba(cr, 0.0, 0.898, 0.627, 0.7); break;
            default:
                cairo_set_source_rgba(cr, 0.7, 0.7, 0.7,
                    0.25 + breathe * 0.2);
        }
        cairo_stroke(cr);
        ...
        double lx = cx + (cell_w - ex.width)/2;
        double ly = cy + cell_h/2 + ex.height/2;

        switch (state) {
            case THREAD_ACTIVE:
                cairo_set_source_rgba(cr, 1.0, 1.0, 1.0, 0.95); break;
            case THREAD_DONE:
                cairo_set_source_rgba(cr, 0.0, 0.898, 0.627, 0.85); break;
            default:
                cairo_set_source_rgba(cr, 0.7, 0.7, 0.7,
                    0.5 + breathe * 0.3);
        }
...
gboolean telemetry_draw_memgauge(GtkWidget *w, cairo_t *cr, App *a) {
    ...
    draw_section_header(cr, 12, 8, W-24, "MEMORY TELEMETRY", 0.0, 0.898, 0.627);

    /* ── draw one arc gauge ── */
    typedef struct { const char *label; double frac; double r,g,b; double used, total; const char *unit; } Gauge;
    double ram_frac  = (a->mem.ram_frac < 0.0 ? 0.0 : (a->mem.ram_frac > 1.0 ? 1.0 : a->mem.ram_frac));
    double vram_frac = (a->mem.vram_frac < 0.0 ? 0.0 : (a->mem.vram_frac > 1.0 ? 1.0 : a->mem.vram_frac));

    Gauge gauges[2] = {
        {"RAM",  ram_frac,  0.0, 0.831, 1.0,   (double)a->mem.ram_used_kb/1024,  (double)a->mem.ram_total_kb/1024,  "MB"},
        {"VRAM", vram_frac, 0.486, 0.227, 0.929, (double)a->mem.vram_used_bytes/1048576, (double)a->mem.vram_total_bytes/1048576, "MB"},
    };

    double gauge_size = (H - 50) * 1.1;
    if (gauge_size > 120) gauge_size = 120;
    double gauge_pad  = (W - 2*gauge_size) / 3.0;

    for (int g = 0; g < 2; g++) {
        Gauge *G = &gauges[g];
        double cx = gauge_pad + g*(gauge_size + gauge_pad) + gauge_size/2;
        double cy = 36 + gauge_size/2;
        double R  = gauge_size/2 - 8;
        ...
        cairo_set_source_rgba(cr, 0.7, 0.7, 0.7, 0.35);
        ...
        if (G->frac > 0.001) {
            ...
            /* tip glow */
            double tx = cx + R * cos(fill_end), ty = cy + R * sin(fill_end);
            draw_glow(cr, tx, ty, 14, G->r, G->g, G->b,
                      0.15 + 0.1 * sin(tick * 0.1));
        }
        ...
        cairo_move_to(cr, cx - lex.width/2, cy + pex.height/2 + 13);
        cairo_show_text(cr, G->label);

        /* used/total */
        if (G->total > 0) {
            char info[32];
            g_snprintf(info, sizeof(info), "%.0f / %.0f %s", G->used, G->total, G->unit);
            cairo_set_font_size(cr, 7.5);
            cairo_text_extents_t iex; cairo_text_extents(cr, info, &iex);
            cairo_set_source_rgba(cr, 0.7, 0.7, 0.7, 0.7);
            cairo_move_to(cr, cx - iex.width/2, cy + pex.height/2 + 24);
...
gboolean telemetry_draw_graph(GtkWidget *w, cairo_t *cr, App *a) {
    ...
    cairo_set_source_rgba(cr, CLR_BORDER, 0.3);
    cairo_set_line_width(cr, 1.0);
    cairo_move_to(cr, pl, pt+gh);
    cairo_line_to(cr, pl+gw, pt+gh - (maxy/maxy)*gh);
    cairo_stroke(cr);

    if (a->serial_time > 0 && a->cpu_time > 0) {
        double sp = a->serial_time / a->cpu_time;

        /* synthetic curve: 1 → sp*0.55 → sp → sp*1.25 → sp*1.4 */
        double sps[5] = {1.0, sp*0.55, sp, sp*1.25, sp*1.4};

        /* area fill under CPU line */
        ...
        if (a->gpu_time > 0) {
            ...
            for (int i = 1; i < 5; i++) {
                ...
                cairo_set_source_rgba(cr, CLR_WHITE, 0.9); cairo_fill(cr);
            }
        }
...
/* ═══════════════════════════════════════════════════════════════════════════
   6.  PERFORMANCE MATRIX — Time vs Dataset Size
   ═══════════════════════════════════════════════════════════════════════════ */
gboolean telemetry_draw_matrix(GtkWidget *w, cairo_t *cr, App *a) {
    ...
    /* illustrative reference curves (serial vs parallel vs GPU)     */
    /* use real data points if available, else draw placeholder curve */
    int have_data = (a->serial_time > 0 && a->cpu_time > 0);

    /* synthetic curve points: log-linear growth */
    double sizes[6]  = {1, 10, 100, 500, 1000, 2000};  /* MB */
    ...
    for (int i = 0; i <= y_divs; i++) {
        double gy = pt + gh - (double)i/y_divs * gh;
        cairo_set_source_rgba(cr, CLR_BORDER, 0.25);
        cairo_set_line_width(cr, 0.5);
        cairo_move_to(cr, pl, gy); cairo_line_to(cr, pl+gw, gy); cairo_stroke(cr);
        char lb[16]; snprintf(lb, sizeof(lb), "%.2fs", (double)i/y_divs * max_time);
        cairo_text_extents_t ex; cairo_text_extents(cr, lb, &ex);
        cairo_set_source_rgba(cr, CLR_SLATE, 0.55);
        cairo_move_to(cr, pl-ex.width-5, gy+3); cairo_show_text(cr, lb);
    }

    /* draw a line for each mode */
    ...
    int n_curves = (a->gpu_time > 0 || !have_data) ? 3 : 2;

    for (int c = 0; c < n_curves; c++) {
        Curve *cv = &curves[c];

        /* area fill for serial */
        if (c == 0) {
    ...
    /* "Efficiency Frontier" crossover annotation */
    double cross_x = pl + 0.18*gw;
    double cross_y = pt + 0.35*gh;
...
/* ═══════════════════════════════════════════════════════════════════════════
   7.  ANIMATION TICK — called every ~16ms from the 60-FPS timer
   ═══════════════════════════════════════════════════════════════════════════ */
void telemetry_tick(App *a) {
    ...
    for (int i = 0; i < MAX_MPI_PROCS; i++) {
        ...
        double diff = target - a->procs[i].progress_display;
        a->procs[i].progress_display += diff * 0.14;
    }

    /* ── thread heatmap: only simulate if no real strategy received yet ── */
    if (a->is_running && a->num_threads == 0) {
        /* Before first strategy packet — run a small 4-thread preview wave */
        int preview = 4;
    ...
    /* ── decay stale active threads (applies even with real data) ── */
    if (a->num_threads > 0) {
        ...
        for (int i = 0; i < nt; i++) {
            if (a->thread_state[i] == THREAD_ACTIVE &&
                ...
                /* 2 second timeout — process finished but no "done" was sent */
                a->thread_state[i] = THREAD_IDLE;
            }
        }
    }

    /* ── idle breathing (sine per thread, phase-shifted) ── */
    for (int i = 0; i < MAX_THREADS; i++) {
        if (a->thread_state[i] == THREAD_IDLE) {
            double ph = a->global_phase + i * 0.31;
            a->thread_breathe[i] = 0.5 + 0.5 * sin(ph);
        } else {
            a->thread_breathe[i] = 0.0;
        }
    }

    /* ── update RAM telemetry from /proc/meminfo ── */
    {
        FILE *mf = fopen("/proc/meminfo", "r");
        if (mf) {
            uint64_t total=0, free_k=0, avail=0;
            char line[128];
            while (fgets(line, sizeof(line), mf)) {
                if (sscanf(line, "MemTotal: %llu kB",     (unsigned long long*)&total) == 1) {}
                if (sscanf(line, "MemAvailable: %llu kB", (unsigned long long*)&avail) == 1) {}
            }
            fclose(mf);
            if (total > 0) {
                free_k = avail;
                a->mem.ram_total_kb = total;
                a->mem.ram_used_kb  = total - free_k;
                a->mem.ram_frac     = (double)(total - free_k) / total;
            }
        }
    }
    ...
    if (a->strategy.gpu_vram_bytes > 0 && a->is_running &&
        ...
        /* rough estimate: chunk_size bytes in flight */
        a->mem.vram_total_bytes = a->strategy.gpu_vram_bytes;
        a->mem.vram_used_bytes  = a->strategy.chunk_size * 2;   /* in+out buffers */
        a->mem.vram_frac = (double)a->mem.vram_used_bytes /
                           a->mem.vram_total_bytes;
        a->mem.vram_frac = CLAMP_D(a->mem.vram_frac, 0.0, 1.0);
    }
}
